...
function renderPCP(){
  ...
  solutions.forEach((sol, si) => {
    ...
    ctx.strokeStyle = OBJ_COLORS[si % OBJ_COLORS.length] + '30';
    ctx.lineWidth = 1.5;
    ctx.stroke();
  });
  ...
  OBJ_LABELS.forEach((lbl, i) => {
    ...
    ctx.fillText(unit, x, MT - 16);

    // Min/max right-aligned to axis
    ctx.fillStyle = '#bbb'; ctx.font = '9px sans-serif'; ctx.textAlign = 'right';
    ctx.fillText(fmt(axMin[i]), x - 8, MT + IH + 4);
    ctx.fillText(fmt(axMax[i]), x - 8, MT + 4);
    ...
    ctx.fillText(fmt(hiSol.objectives[lbl]), x, hy - 12);
  });

  // Click — nearest line segment or dot
  cv.onclick = e => {
    ...
    solutions.forEach((sol, si) => {
      ...
      for(let j = 0; j < nDims - 1; j++){
        const x1 = xOf(j),   y1 = yOf(j,   sol.objectives[OBJ_LABELS[j]]);
        const x2 = xOf(j+1), y2 = yOf(j+1, sol.objectives[OBJ_LABELS[j+1]]);
